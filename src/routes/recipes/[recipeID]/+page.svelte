...
<div class={`flex flex-col gap-2.5 ${dialogView || 'p-8 size-full items-center content-center'}`}>
	...
	<div class="flex flex-col gap-1 p-1 {dialogView ? 'w-full' : 'md:w-96 w-full'}">
		{#each data.recipe?.steps as step (step.id)}
			<div class="flex flex-row gap-2.5 items-center justify-start content-center bg-slate-100 rounded-xl p-2.5">
